...
writeOGR(hh.utm, "data/WHSA_hh_UTM30.shp", "WHSA_hh", "ESRI Shapefile")

hh.utm <- as.data.frame(hh.utm)
qplot(hh.utm$x, hh.utm$y, xlab="Easting", ylab="Northing", main="WHSA HH in UTM30")
ggsave("data/household_positions.png", width=WIDTH, height=HEIGHT, dpi=DPI)
write.csv(hh.utm, file="data/WHSA_hh_UTM30.csv", row.names=F)
